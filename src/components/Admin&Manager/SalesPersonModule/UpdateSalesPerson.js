  ...
  const salesPersonRow=JSON.parse(localStorage.getItem('salesPersonRow'))
    ...
    const clearHandler = () => {
      if (window.confirm('Are you sure to clear fields?')) setSales_person_details({
        maxTarget:'',threshold:'',frequency:'',duration:'',currency:''
      })
    }
    const submitHandler = async (e) => {
      const { maxTarget, frequency, threshold, currency, duration } = sales_person_details
      e.preventDefault()
      await api.put(`/app/updateSalesperson/${sales_person_details?.salespersonId}`, { maxTarget, frequency, threshold, currency, duration })
        .then(res => {
